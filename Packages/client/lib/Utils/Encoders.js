...
export function hexDecode(encoded) {
    const bytes = new Uint8Array(encoded.match(/.{1,2}/g).map(byte => parseInt(byte, 16)));
    return new TextDecoder().decode(bytes);
}
...
export function binaryDecode(encoded) {
    const bytes = new Uint8Array(encoded.match(/.{1,8}/g).map(bin => parseInt(bin, 2)));
    return new TextDecoder().decode(bytes);
}
...
export function latin1Encode(input) {
    const bytes = Uint8Array.from(input.split('').map(c => c.charCodeAt(0) & 0xff));
