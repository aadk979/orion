...
async function checkAndDeployCaptcha(serverURL, nameSpace, slug, customStyles = {}) {
    const headers = await buildNoAuthHeaders();
    const req = await fetchWithTimeout(`${serverURL}${slug !== '' ? '/' + slug : ''}/${nameSpace}/api/v1/request/have-no-auth-token`, {
        method: 'POST',
        headers,
        credentials: 'include'
    });
