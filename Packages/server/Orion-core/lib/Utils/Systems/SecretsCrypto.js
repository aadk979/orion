...
class SecretsCrypto {
    ...
    async generateECDSAKey(config) {
        ...
        const hashedKeys = hashStringSync(`${exportedKeys.public.base64}${exportedKeys.private.base64}${date}${keyPairId}`, 'sha256');
    ...
    async importECDSAPublicKeyFromBase64(base64PublicKey, namedCurve = 'P-256') {
        ...
        const publicKey = await crypto.subtle.importKey('spki', binaryDer, { name: 'ECDSA', namedCurve }, true, ['verify']);
    ...
    async importRSAPublicKeyFromBase64(base64PublicKey, hash = 'SHA-256') {
        ...
        const publicKey = await crypto.subtle.importKey('spki', binaryDer, { name: 'RSASSA-PKCS1-v1_5', hash }, true, ['verify']);
    ...
    async importRSAPublicKeyFromJWK(jwk) {
        ...
        const publicKey = await crypto.subtle.importKey('jwk', jwk, { name: 'RSASSA-PKCS1-v1_5', hash }, true, ['verify']);
    ...
    async importECDSAPrivateKeyFromBase64(base64PrivateKey, namedCurve = 'P-256') {
        ...
        const privateKey = await crypto.subtle.importKey('pkcs8', binaryDer, { name: 'ECDSA', namedCurve }, true, ['sign']);
    ...
    async importRSAPrivateKeyFromBase64(base64PrivateKey, hash = 'SHA-256') {
        ...
        const privateKey = await crypto.subtle.importKey('pkcs8', binaryDer, { name: 'RSASSA-PKCS1-v1_5', hash }, true, ['sign']);
    ...
    async importECDSAPrivateKeyFromJWK(jwk) {
        const privateKey = await crypto.subtle.importKey('jwk', jwk, { name: 'ECDSA', namedCurve: jwk.crv }, true, ['sign']);
    ...
    async importRSAPrivateKeyFromJWK(jwk) {
        ...
        const privateKey = await crypto.subtle.importKey('jwk', jwk, { name: 'RSASSA-PKCS1-v1_5', hash }, true, ['sign']);
