...
const accessKey = `TOKEN_SECRETS_MANAGER_${domain}`;
...
async function generateAccessToken(uid, email, fingerprint, authMethod, role, ip, userAgent, accessTokenLinkCodeExternal) {
    ...
    const secret = await globalAccessPoint.getValue(accessKey).getRandomSigningKeyPair();
    ...
    if (securityTier === 'TIER_1') {
        payload.jti = generateId('ACCESS_TOKEN', 15);
...
async function validateAccessToken(token, fingerprint, ip, clientUrl) {
    ...
    try {
        ...
        const secret = await globalAccessPoint.getValue(accessKey).findKeyPair(decodedHeader.kid);
