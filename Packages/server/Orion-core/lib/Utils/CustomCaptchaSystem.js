...
function addComplexNoise(ctx, width, height) {
    ...
    for (let x = 0; x < width; x += 2) {
        for (let y = 0; y < height; y += 2) {
            const noise = Math.sin(x * 0.02) * Math.cos(y * 0.03) * Math.sin((x + y) * 0.01) * 127 + 128;
...
function renderCharacter(ctx, char, x, y, index, totalChars) {
    const fonts = ['bold 38px Courier New', 'bold 36px monospace', 'bold 40px serif', 'bold 37px Georgia'];
...
export { generateCaptchaImage, verifyCaptcha, generateChallengeToken, createRateLimitKey };
