...
const routeHandlerInitiatePasswordReset = async (request, response) => {
    ...
    if (callback.reqId) {
        response.cookie("passwordResetRequestId", stringifyCookieData(callback.reqId), { httpOnly: true, secure: true, sameSite: "None", maxAge: 15 * 60 * 1000 });
    }
...
export {
    ...
    verifyPasswordResetCodeAndUpdate,
    routeHandlerInitiatePasswordReset,
    routeHandlerCompletePasswordReset
};