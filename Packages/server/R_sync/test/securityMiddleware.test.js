...
} from "../lib/core/middleware/securityMiddleware.js";
import {
...
describe("securityMiddleware time helpers", () => {
  it("accepts timestamps within leeway (seconds)", () => {
    ...
    assert.strictEqual(
      isTimestampWithinLeeway(now, now, AUTH_TIMESTAMP_LEEWAY_SEC),
      true,
    );
    assert.strictEqual(
      isTimestampWithinLeeway(now - 10, now, AUTH_TIMESTAMP_LEEWAY_SEC),
      true,
    );
    assert.strictEqual(
      isTimestampWithinLeeway(now - 100, now, AUTH_TIMESTAMP_LEEWAY_SEC),
      false,
    );
  });
  ...
  it("registration PoP (nonce + key-bound) matches between client and middleware", () => {
    ...
    assert.strictEqual(
      verifySignature(middlewarePayload, sig, identity.publicKey),
      true,
    );
  });
  ...
  it("registration PoP fails if the encryption key is swapped", () => {
    ...
    assert.strictEqual(
      verifySignature(tamperedPayload, sig, identity.publicKey),
      false,
    );
  });
});
