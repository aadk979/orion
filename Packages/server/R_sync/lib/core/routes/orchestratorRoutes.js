...
import { restrictToLocalhost, enforceETSLockdown, validateWorkerSignature, validateRegistrationSignature } from '../middleware/securityMiddleware.js';

const router = express.Router();

// Worker discovery/registration (Protected by PoP + ETS lockdown guard)
router.post('/discover-me', enforceETSLockdown, validateRegistrationSignature, registerWorker);
...
router.post('/worker-event', enforceETSLockdown, validateWorkerSignature, handleWorkerEvent);
...
router.post('/heartbeat', enforceETSLockdown, validateWorkerSignature, heartbeat);
