...
class TunnelManager {
  ...
  async initialize() {
    ...
    // Generate orchestrator's ECC key pair for ECDH
    this.keyPair = await generateKeyPairECC(this.encryptionAlg);

    // Generate Ed25519 signature key pair
    this.signatureKeyPair = generateSignatureKeyPair();
  ...
  async establishTunnel(
    ...
    try {
      ...
      const workerPublicKey = await importPublicKeyECC(
        new Uint8Array(workerPublicKeyRaw),
        this.encryptionAlg,
      );
      ...
      const orchestratorPublicKeyRaw = await exportPublicKeyECC(
        this.keyPair.publicKey,
      );
      const tunnelSalt = deriveTunnelSalt(
        new Uint8Array(workerPublicKeyRaw),
        orchestratorPublicKeyRaw,
      );
      ...
      logger.error(
        `Failed to establish tunnel with worker ${workerId}: ${err.message}`,
      );
      return false;
    }
  }
  ...
  async sendToWorker(workerId, event) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        logger.info(
          `Sending event "${event.name}" to worker ${workerId} (attempt ${attempt}/${MAX_RETRIES})...`,
        );
        ...
        logger.info(
          `Event "${event.name}" confirmed by worker ${workerId} on attempt ${attempt}`,
        );
        return true;
      } catch (err) {
        lastError = err;
        logger.warn(
          `Attempt ${attempt}/${MAX_RETRIES} failed for worker ${workerId}: ${err.message}`,
        );
    ...
    logger.error(
      `All ${MAX_RETRIES} attempts failed for worker ${workerId}. Marking as faulty.`,
    );
    ...
    try {
      ...
      logger.error(
        `Failed to persist faulty status for worker ${workerId}: ${updateErr.message}`,
      );
    }
  ...
  async broadcastToAll(event) {
    ...
    logger.info(
      `Broadcasting "${event.name}" to ${activeWorkers.length} active workers...`,
    );
    ...
    logger.info(
      `Broadcast complete: ${successCount}/${activeWorkers.length} workers successfully reached`,
    );
