...
class R_Sync {
  ...
  async startOrchestrator() {
    ...
    let existingConfig = await readFromCaller(
      PERSISTANT_ORCHESTRATOR_CONFIG_FILE,
    );
    ...
    const tunnelManager = getTunnelManager(
      `P-${this.encryptionAlg.split("_")[1]}`,
    );
    ...
    globalAccessPoint.setValue(
      "trustAdvertisedWorkerIp",
      this.trustAdvertisedWorkerIp === true,
    );
    ...
    this.app.use(
      `/${globalAccessPoint.nameSpace()}/api/v1`,
      orchestratorRoutes,
    );
  ...
  async startWorker() {
    ...
    const encryptionKeyPair = await generateKeyPairECC(
      `P-${this.encryptionAlg.split("_")[1]}`,
    );
    const signatureKeyPair = generateSignatureKeyPair();
    const exportedPublicKey = await exportPublicKeyECC(
    ...
    const orchestratorPublicKey = await importPublicKeyECC(
      new Uint8Array(data.orchestratorEncryptionPublicKey),
      `P-${this.encryptionAlg.split("_")[1]}`,
    );
    ...
    const tunnelSalt = deriveTunnelSalt(
      exportedPublicKey,
      new Uint8Array(data.orchestratorEncryptionPublicKey),
    );
  ...
  async emitToOrchestrator(eventName, data = {}) {
    ...
    const payloadSignature = generateSignature(
      encrypted,
      tunnelState.mySignatureKeyPair.privateKey,
    );
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const authSignature = generateSignature(
          authPayload,
          tunnelState.mySignatureKeyPair.privateKey,
        );

        logger.info(
          `Emitting "${eventName}" to orchestrator (attempt ${attempt}/${MAX_RETRIES})...`,
        );
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            `Orchestrator returned HTTP ${response.status}: ${errorData.message || response.statusText}`,
          );
        }
        ...
        logger.info(
          `Event "${eventName}" confirmed by orchestrator on attempt ${attempt} (${result.eventId})`,
        );
        return result;
      } catch (err) {
        lastError = err;
        logger.warn(
          `Attempt ${attempt}/${MAX_RETRIES} failed to emit "${eventName}" to orchestrator: ${err.message}`,
        );
    ...
    logger.error(
      `All ${MAX_RETRIES} attempts to emit "${eventName}" to orchestrator failed.`,
    );
    throw lastError;
  }
