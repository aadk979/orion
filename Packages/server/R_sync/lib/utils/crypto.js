...
/**
 * Derive a deterministic per-tunnel HKDF salt from both parties' raw ECDH
 * public keys. Both sides compute this identically (worker bytes first, then
 * orchestrator bytes) so they arrive at the same AES key while still getting
 * per-session domain separation instead of a fixed constant salt.
 */
function deriveTunnelSalt(workerPubBytes, orchestratorPubBytes) {
...
async function deriveKey(sharedSecret, salt = new Uint8Array(16), info = new Uint8Array(0)) {
    const keyMaterial = await crypto.webcrypto.subtle.importKey('raw', sharedSecret, 'HKDF', false, ['deriveKey']);
