...
class ErrorTrackerSystem {
    ...
    /**
     * Produce a compact summary of the current tracker state. Safe to embed in
     * an audit record (unlike massExport(), which can be very large).
     */
    getSummarySnapshot() {
        return {
            totalErrors: this._errorsTracked_errors.length,
            distinctMessages: this._errorCountByMessage.size,
            distinctFunctions: this._errorCountByFunction.size,
            distinctSources: this._errorCountBySource.size,
            topErrorMessages: this.getTopErrorMessages(3),
            mostErrorProneFunctions: this.getMostErrorProneFunctions(3),
            snapshotAt: new Date().toISOString()
        };
    }

    /**
     * Clear all tracked error state. Intended to be called when an operator
     * lifts a lockdown, so the cumulative TOTAL_ERRORS threshold does not
     * immediately re-trip the lockdown on the next error. Callers should snapshot
     * (getSummarySnapshot / massExport) beforehand if the data must be retained.
     */
    reset() {
        this._errorsTracked_errors = [];
        this._errorsTracked_functionName.clear();
        this._errorsTracked_functionSource.clear();
        this._errorsTracked_errorMessage.clear();
        this._errorsTracked_errStack.clear();
        this._errorCountByMessage.clear();
        this._errorCountByFunction.clear();
        this._errorCountBySource.clear();
        this._errorRelations.clear();
        this._recentErrors = [];
    }
