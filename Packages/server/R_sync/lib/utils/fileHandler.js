...
async function deleteFromCaller(filename) {
    try {
        const callerDir = getCallerDirectory();
        if (!callerDir) return { error: true, errorCode: 'CALLER-DIRECTORY-NOT-FOUND' };
        ...
        try {
            ...
            return { error: false, deleted: false, message: 'File does not exist' };
        }

        await fs.promises.unlink(targetPath);
        return { error: false, deleted: true, path: targetPath };
    } catch (err) {
        if (err.code === 'EACCES') return { error: true, errorCode: 'PERMISSION-DENIED' };
        if (err.code === 'ENOENT') return { error: false, deleted: false };
        return { error: true, errorCode: 'DELETE-OPERATION-FAILED' };
    }
}

export { writeToCaller, readFromCaller, deleteFromCaller };
