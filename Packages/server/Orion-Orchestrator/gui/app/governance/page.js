    ...
    return (
        <>
            ...
            <div className="panel">
                ...
                <p className="sub">
                    Passwordless — the invite email carries their first magic link; TOTP enrollment is mandatory before the account activates. With no policy or
                    group selected, the built-in read-only policy is attached.
                </p>
                <div className="row" style={{ alignItems: 'flex-end' }}>
                    ...
                    <button
                        disabled={!email}
                        onClick={act(
                            () => post('/api/admins', { email, displayName: name || null, policyId: policyId || null, groupId: groupId || null }),
                            `Admin ${email} created and invited`
                        )}
                    >
                        Create &amp; invite
                    </button>
                    ...
                    <tbody>
                        ...
                        {admins?.map(a => (
                            <tr key={a.id}>
                                ...
                                <td>
                                    <span className={`badge ${a.role === 'root' ? 'crit' : 'info'}`}>{a.role}</span>
                                </td>
                                <td>
                                    <span className={`badge ${a.status === 'active' ? 'ok' : a.status === 'suspended' ? 'crit' : 'warn'}`}>{a.status}</span>
                                </td>
                                        ...
                                        <div className="row">
                                            {a.status !== 'suspended' ? (
                                                <button
                                                    ...
                                                    onClick={act(() => post(`/api/admins/${a.id}/status`, { status: 'suspended' }), `${a.email} suspended`)}
                                                >
                                                    Suspend
                                                </button>
                                            ) : (
                                                <button
                                                    className="ghost small"
                                                    onClick={act(() => post(`/api/admins/${a.id}/status`, { status: 'active' }), `${a.email} reinstated`)}
                                                >
                                                    Reinstate
                                                </button>
                                            )}
            ...
            <div className="panel">
                ...
                <div className="row">
                    <label className="field" style={{ flex: 1 }}>
                        <span>Name</span>
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="ops-oncall" />
                    </label>
                ...
                <label className="field">
                    <span>Policy document (deny overrides; default deny)</span>
                    <textarea className="mono" value={doc} onChange={e => setDoc(e.target.value)} />
                </label>
                <button
                    disabled={!name}
                        ...
                        {policies?.map(p => (
                            <tr key={p.id}>
                                <td className="mono">
                                    ...
                                    {p.managed && (
                                        <div>
                                            <span className="badge dim">built-in</span>
                                        </div>
                                    )}
                                    <div className="sub" style={{ fontSize: 11 }}>
                                        {p.id}
                                ...
                                <td>{p.description || '—'}</td>
                                <td>
                                    <pre className="json" style={{ maxHeight: 140, maxWidth: 420 }}>
                                        {JSON.stringify(p.document, null, 1)}
                                    </pre>
                                </td>
                                <td>
                                    {!p.managed && (
        ...
        <>
            ...
            <div className="panel">
                ...
                <div className="row" style={{ alignItems: 'flex-end' }}>
                    <label className="field" style={{ flex: 1, marginBottom: 0 }}>
                        <span>Name</span>
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="sre-oncall" />
                    </label>
                    <label className="field" style={{ flex: 2, marginBottom: 0 }}>
                        <span>Description</span>
                        <input value={description} onChange={e => setDescription(e.target.value)} />
            ...
            {groups?.map(g => (
                <div className="panel" key={g.id}>
                    <div className="row" style={{ justifyContent: 'space-between' }}>
                        ...
                        <div className="row">
                            <button className="ghost small" onClick={() => loadMembers(g.id)}>
                                Show members
                            </button>
                            <button
                                className="danger small"
                            ...
                            <table>
                                <tbody>
                                    {members[g.id].length === 0 && (
                                        <tr>
                                            <td className="sub">No members.</td>
                                        </tr>
                                    )}
                                    {members[g.id].map(m => (
                                        <tr key={m.id}>
                                            ...
                                            <td>
                                                <span className="badge dim">{m.status}</span>
                                            </td>
                                            <td>
                                                <button
                                                    className="ghost small"
