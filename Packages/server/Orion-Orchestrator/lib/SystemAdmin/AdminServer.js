...
const PENDING_ACCOUNT_ALLOWED = new Set([
    ...PENDING_STAGE_ALLOWED,
    'POST /api/auth/root/change-password'
]);
...
class AdminServer {
    ...
    async start() {
        ...
        logger.info(`AdminServer: orch panel + API listening on ${this.config.host}:${this.config.port}${existsSync(GUI_DIR) ? '' : ' (GUI build not found — API only)'}`);
    }
    ...
    _mountAuthRoutes() {
        ...
        router.post('/magic-link', limited, this._h(async (req, res) => {
        ...
        router.post('/magic-link/verify', limited, this._h(async (req, res) => {
        ...
        router.post('/root/login', limited, this._h(async (req, res) => {
            const result = await this.service.rootLogin(req.body?.email, req.body?.password, this._ip(req), req.headers['user-agent']);
            this._setSessionCookie(res, result.token, this.service.config.pendingSessionTtlMinutes * 60);
            res.json({ error: false, ...result });
        }));

        router.post('/totp/setup', this._requireSession(), this._h(async (req, res) => {
            res.json({ error: false, ...await this.service.totpSetupStart(req.adminSession) });
        }));

        router.post('/totp/activate', limited, this._requireSession(), this._h(async (req, res) => {
        ...
        router.post('/totp/verify', limited, this._requireSession(), this._h(async (req, res) => {
        ...
        router.post('/root/change-password', limited, this._requireSession(), this._h(async (req, res) => {
            const result = await this.service.rootChangePassword(
                req.adminSession, req.body?.currentPassword, req.body?.newPassword, this._ip(req)
            );
            res.json({ error: false, ...result });
        }));
        ...
        router.post('/logout', this._requireSession(), this._h(async (req, res) => {
    ...
    _mountClusterRoutes() {
        ...
        // Reads
        router.get('/status', this._gate(AdminActions.READ_STATUS), this._h(async (req, res) => {
        ...
        router.get('/nodes/:workerId', this._gate(AdminActions.READ_NODES, req => nodeResource(req.params.workerId)), (req, res) => {
        ...
        router.post('/nodes/:workerId/command',
            this._gate(req => commandAction(String(req.body?.action || 'unknown')), req => nodeResource(req.params.workerId)),
            this._h(async (req, res) => {
                const { action, args, timeoutMs } = req.body || {};
                if (!isKnownCommand(action)) {
                    return res.status(400).json({ error: true, code: 'CLUSTER::UNKNOWN-COMMAND', message: `"${action}" is not an allowlisted cluster command` });
                }
                const outcome = await this.orch.command(
                    req.params.workerId, action, args || {},
        ...
        // Cluster operations
        router.post('/lock', this._gate(AdminActions.OPS_LOCK_CLUSTER), this._h(async (req, res) => {
            res.json({ error: false, results: await this.orch.lockCluster(this._actor(req)) });
        }));

        router.post('/unlock', this._gate(AdminActions.OPS_UNLOCK_CLUSTER), this._h(async (req, res) => {
            res.json({ error: false, results: await this.orch.unlockCluster(this._actor(req)) });
        }));

        router.post('/incident/declare', this._gate(AdminActions.OPS_DECLARE_INCIDENT), this._h(async (req, res) => {
        ...
        router.post('/incident/resolve', this._gate(AdminActions.OPS_RESOLVE_INCIDENT), this._h(async (req, res) => {
        ...
        router.post('/consensus', this._gate(AdminActions.OPS_PROPOSE_CONSENSUS), this._h(async (req, res) => {
        ...
        router.post('/client-urls', this._gate(AdminActions.OPS_ADD_CLIENT_URLS), this._h(async (req, res) => {
    ...
    _mountGovernanceRoutes() {
        ...
        policies.post('/:id/detach', this._h(async (req, res) => {
            res.json({
                error: false,
    ...
    _mountAuditRoutes() {
        ...
        router.get('/', this._gate(AdminActions.READ_AUDIT, 'audit'), this._h(async (req, res) => {
        ...
        router.get('/verify', this._requireRoot(), this._h(async (req, res) => {
    ...
    _mountGui() {
        ...
        if (existsSync(GUI_DIR)) {
            ...
            this.app.use((req, res, next) => {
                ...
                res.sendFile(join(GUI_DIR, 'index.html'));
            });
        }
    }
